...
const Signup = () => {
    ...
    // Agency name is now stored in the database
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            // Backend requires: email, username, password, role, full_name (optional), agency_name (optional)
            ...
            const payload = {
                ...
                role: isAgency ? "manager" : "client" // "manager" for Agency, "client" for Personal
            };

            const result = await authService.register(payload);
            // After successful registration and auto-login, navigate to dashboard
            if (result.access_token || localStorage.getItem('token')) {
                navigate('/dashboard');
            } else {
                // Fallback to login if auto-login didn't work
                navigate('/login');
            }
        } catch (err) {
            console.error(err);
    ...
    return (
        <div className="min-h-screen flex bg-white">
            {/* Left Side - Gradient (On Signup it is Left) */}
